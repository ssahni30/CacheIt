...
import com.cache.service.CronJobs;
...
public class CacheItProcessorImpl implements CacheItProcessor {
    ...
    public Map<String, MapData> initialization() {
        ...
        for (Class<?> clazz : annotatedClasses) {
            ...
            if(!clazz.getAnnotation(CacheIt.class).cron().equals("")){
                initializeCron(mapName, clazz.getAnnotation(CacheIt.class).cron());
            }
        }
    ...
    private void initializeCron(String mapName, String cron) {
        // define the job and tie it to our MyJob class
        JobDetail job = JobBuilder.newJob(CronJobs.class)
                .withIdentity(mapName, "cacheRefresh")
                .build();

        // Trigger the job to run now, and then repeat every 40 seconds
        Trigger trigger = TriggerBuilder.newTrigger()
                .withIdentity(mapName + "_trigger", "cacheRefresh")
                .startNow()
